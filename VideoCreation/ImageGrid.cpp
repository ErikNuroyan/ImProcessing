...
void ImageGrid::resize_once_expand(int n, cv::VideoWriter &v, const cv::Mat & upper, const cv::Mat & lower) {
	this->energy();
	Node * current;
	Node * prev_current;
...
void ImageGrid::resize(int n_pixels,  cv::VideoWriter & v, const cv::Mat & upper, const cv::Mat & lower) {
	if (n_pixels < 0) {
		cv::Mat result = cv::Mat(upper.rows + this->height + lower.rows, upper.cols, CV_8UC3);
		for (int i = 0; i < upper.rows - 1; i++) {
			for (int j = 0; j < upper.cols; j++) {
				result.at<cv::Vec3b>(i, j) = upper.at<cv::Vec3b>(i, j);
			}
		...
		for (int i = 0; i < abs(n_pixels); i++) {
			...
			for (int k = upper.rows + intermediate.rows + lower.rows; k < result.rows; k++) {
				for (int w = 0; w < result.cols; w++) {
					result.at<cv::Vec3b>(k, w) = cv::Vec3b(0, 0, 0);
				}
			}
			v.write(result);
		}
		...
		resize_once_expand(n_pixels, v, upper, lower);
	}
	else {
		return;
	}
